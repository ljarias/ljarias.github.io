...
def extract_news(markdown: str) -> list[dict]:
    ...
    for number, title, block in re.findall(pattern, markdown, flags=re.MULTILINE | re.DOTALL | re.IGNORECASE):
        ...
        items.append({
            ...
            "topic": short(tema.group(1), 44) if tema else "IA",
...
def extract_semaphore(markdown: str) -> list[str]:
    ...
    if not match:
        return ["Control y ciberseguridad", "Empleo y regulación", "Educación y acceso"]
    lines = []
    ...
    return (lines + ["Empleo y regulación", "Educación y acceso"])[:3]
...
def source_names(markdown: str) -> list[str]:
    ...
    pretty = {
        ...
        "bbc.co.uk": "BBC", "nature.com": "Nature", "science.org": "Science",
    ...
    for domain in domains:
        ...
        if name not in names:
            names.append(name)
    return names[:5] or ["Fuentes enlazadas en la edición diaria"]
...
def build_svg(news: list[dict], semaphore: list[str], sources: list[str], human_date: str) -> str:
    out = [
        ...
        f'<text x="50" y="151" font-family="Arial,Helvetica,sans-serif" font-size="28" font-weight="700" fill="#FFFFFF">{esc(human_date)}</text>',
        '<rect x="42" y="178" width="710" height="62" rx="24" fill="#FFD52A"/>',
        '<text x="68" y="219" font-family="Arial,Helvetica,sans-serif" font-size="28" font-weight="800" fill="#0C2F4E">Qué preocupa hoy a expertos y gobiernos</text>',
        '<text x="48" y="285" font-family="Arial,Helvetica,sans-serif" font-size="23" font-style="italic" fill="#163A59">La inteligencia artificial avanza rápido.</text>',
        '<text x="48" y="316" font-family="Arial,Helvetica,sans-serif" font-size="23" font-style="italic" fill="#163A59">Estas son las noticias clave de hoy.</text>',
    ...
    for idx, item in enumerate(news[:6]):
        ...
        out += [
            ...
            f'<rect x="{x+122}" y="{y+239}" width="{min(330, 22 + len(topic)*9)}" height="30" rx="15" fill="{accent}" opacity=".95"/>',
    ...
    out += [
        ...
        '<text x="62" y="1318" font-family="Arial" font-size="30" font-weight="900" fill="#FFFFFF">3 ideas clave para el aula</text>',
    ]
    ideas = [
        ("1", "Capacidad ≠ control", "Que un sistema sea potente no significa que sea totalmente predecible."),
        ("2", "Innovar exige reglas", "Más autonomía requiere seguridad, supervisión y responsabilidades claras."),
        ("3", "Advertencia ≠ hecho", "Distingue datos confirmados, opiniones, proyecciones y escenarios hipotéticos."),
    ]
    idea_x = [48, 264, 480]
    ...
    out += [
        ...
        '<text x="738" y="1318" font-family="Arial" font-size="28" font-weight="900" fill="#FFFFFF">Semáforo del día</text>',
    ]
    lights = [("#EF4444", semaphore[0], "Mayor atención"), ("#F2B705", semaphore[1], "En observación"), ("#10B981", semaphore[2], "Oportunidad")]
    yy = [1383, 1474, 1565]
    ...
    out += [
        ...
        '<text x="48" y="1785" font-family="cursive" font-size="37" font-style="italic" fill="#FFFFFF">Informarnos hoy,</text>',
        '<text x="48" y="1832" font-family="cursive" font-size="37" font-style="italic" fill="#FFFFFF">para decidir mejor mañana.</text>',
        ...
        '<text x="1030" y="1840" text-anchor="end" font-family="Arial" font-size="15" fill="#D6E7F6">Resumen educativo · Observatorio IA al Día</text>',
...
def main() -> None:
    ...
    while len(news) < 6:
        news.append({"number": str(len(news)+1), "title": "Tema para seguir", "type": "OBSERVACIÓN", "topic": "Seguimiento", "summary": "Revisa la edición diaria para ampliar el contexto y consultar las fuentes enlazadas."})

    svg = build_svg(news[:6], extract_semaphore(markdown), source_names(markdown), human_date)
